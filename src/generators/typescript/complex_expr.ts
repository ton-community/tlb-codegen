...
export function storeTagExpression(tag: TLBConstructorTag): Statement {
    return tExpressionStatement(
        tFunctionCall(tMemberExpression(id('builder'), id('storeUint')), [id(tag.binary), tNumericLiteral(tag.bitLen)]),
    );
}
export function storeFunctionStmt(statements: Statement[]): Statement {
    return tReturnStatement(tArrowFunctionExpression([tTypedIdentifier(id('builder'), id('Builder'))], statements));
}
export function checkKindStmt(
...
export function storeFunctionParam(varName: string): TypedIdentifier {
    return tTypedIdentifier(
        id('store' + varName),
        tArrowFunctionType(
            [tTypedIdentifier(id(firstLower(varName)), id(varName))],
...
export function loadFunctionParam(varName: string): TypedIdentifier {
    return tTypedIdentifier(id('load' + varName), tArrowFunctionType(typedSlice(), id(varName)));
}
export function skipTagStmt(bitLen: number): Statement {
    return tExpressionStatement(
        tFunctionCall(tMemberExpression(id('slice'), id('loadUint')), [tNumericLiteral(bitLen)]),
    );
}
export function checkTagExpr(tag: TLBConstructorTag): BinaryExpression {
    return tEqualExpression(
        tFunctionCall(tMemberExpression(id('slice'), id('preloadUint')), [tNumericLiteral(tag.bitLen)]),
        id(tag.binary),
    );
}
export function checkHasBitsForTag(bitLen: number): BinaryExpression {
    return tBinaryExpression(tMemberExpression(id('slice'), id('remainingBits')), '>=', tNumericLiteral(bitLen));
}
export function storeFunctionExpr(typeParamExpr: TypeExpression, storeExpr2: Statement): Expression | undefined {
    return tArrowFunctionExpression(
        ...
        [tReturnStatement(tArrowFunctionExpression([tTypedIdentifier(id('builder'), id('Builder'))], [storeExpr2]))],
    );
}
export function coverFuncCall(loadExpr: Expression): Expression {
    return loadExpr.type == 'FunctionCall'
        ? tArrowFunctionExpression(typedSlice(), [tReturnStatement(loadExpr)])
        : loadExpr;
}
export function storeExprForParam(
    theCell: string,
...
export function loadExprForParam(currentSlice: string, exprForParam: ExprForParam): Expression {
    return tFunctionCall(
        tMemberExpression(id(currentSlice), id('load' + exprForParam.fieldLoadSuffix)),
...
export function loadFromNewSlice(loadExpr: Expression): Expression {
    return tArrowFunctionExpression(typedSlice(), [sliceLoad([1, 0], 'slice'), tReturnStatement(loadExpr)]);
}
export function arrayedType(typeParamExpr: TypeExpression): TypeExpression {
    return tTypeWithParameters(id('Array'), tTypeParametersExpression([typeParamExpr]));
}
export function storeTupleStmt(
    ...
    return tExpressionStatement(
        tFunctionCall(tMemberExpression(currentParam, id('forEach')), [
            tArrowFunctionExpression([tTypedIdentifier(id('arg'), typeParamExpr)], [storeExpr]),
        ]),
    );
}
export function loadTupleExpr(arrayLength: Expression, loadExpr: Expression): Expression {
    return tFunctionCall(
        tMemberExpression(
            tFunctionCall(tMemberExpression(id('Array'), id('from')), [
                tFunctionCall(tMemberExpression(tFunctionCall(id('Array'), [arrayLength]), id('keys')), []),
            ]),
            id('map'),
        ),
...
export function storeRefStmt(slicePrefix: number[], currentCell: string): Statement {
    return tExpressionStatement(
        tFunctionCall(tMemberExpression(id(currentCell), id('storeRef')), [id(getCurrentSlice(slicePrefix, 'cell'))]),
    );
}
export function newCellStmt(slicePrefix: number[]): Statement {
    return tExpressionStatement(
        tDeclareVariable(id(getCurrentSlice(slicePrefix, 'cell')), tFunctionCall(id('beginCell'), [])),
    );
}
export function checkConstraintStmt(constraintAST: Expression, exceptionCommentLastPart: string): Statement {
    return tIfStatement(tUnaryOpExpression('!', constraintAST), [
        tExpressionStatement(
            id("throw new Error('Condition " + toCode(constraintAST).code + exceptionCommentLastPart + "')"),
        ),
    ]);
}
...
export function negationDerivationFuncDecl(
    ...
    return tFunctionDeclaration(
        ...
        getNegationDerivationFunctionBody(tlbCode, fieldTypeName, argIndex, fieldName),
    );
}
export function dictStoreStmt(
    currentCell: string,
    storeParametersInside: Expression[],
    keyForStore: Expression,
    valueStore: ObjectExpression,
    direct: boolean,
): Statement | undefined {
    return tExpressionStatement(
        tFunctionCall(
            tMemberExpression(id(currentCell), id('storeDict' + (direct ? 'Direct' : ''))),
            storeParametersInside.concat([keyForStore, valueStore]),
        ),
    );
}
export function dictTypeParamExpr(
    fieldType: TLBHashmapType,
    typeParamExpr: TypeExpression,
): TypeExpression | undefined {
    return tTypeWithParameters(
        id('Dictionary'),
        tTypeParametersExpression([isBigIntExpr(fieldType.key) ? id('bigint') : id('number'), typeParamExpr]),
    );
}
export function dictValueStore(
    typeParamExpr: TypeExpression,
    storeFunctionExpr: Expression,
    extraStoreFunctionExpr?: Expression,
) {
    let subStatements = [
        tExpressionStatement(
            tFunctionCall(tFunctionCall(storeFunctionExpr, [id(extraStoreFunctionExpr ? 'arg.value' : 'arg')]), [
                id('builder'),
            ]),
        ),
    ];
    if (extraStoreFunctionExpr) {
        subStatements = [
            tExpressionStatement(
                tFunctionCall(tFunctionCall(extraStoreFunctionExpr, [id('arg.extra')]), [id('builder')]),
            ),
    ...
    return tObjectExpression([
        tObjectProperty(
            id('serialize'),
            tArrowFunctionExpression(
                [tTypedIdentifier(id('arg'), typeParamExpr), tTypedIdentifier(id('builder'), id('Builder'))],
                subStatements,
            ),
        ),
        tObjectProperty(id('parse'), id("() => { throw new Error('Not implemented') }")),
    ]);
}
export function dictLoadExpr(
    keyForLoad: Expression,
    loadFunctionExpr: Expression,
    currentSlice: string,
    direct: boolean,
): Expression | undefined {
    return tFunctionCall(tMemberExpression(id('Dictionary'), id('load' + (direct ? 'Direct' : ''))), [
        keyForLoad,
...
function dictValueLoad(loadFunctionExpr: Expression) {
    return tObjectExpression([
        tObjectProperty(id('serialize'), id("() => { throw new Error('Not implemented') }")),
        tObjectProperty(id('parse'), loadFunctionExpr),
    ]);
}
export function dictKeyExpr(keyType: TLBMathExprType, ctx: ConstructorContext, objectId?: string): Expression {
    ...
    return tFunctionCall(tMemberExpression(id('Dictionary.Keys'), isBigIntExpr(keyType) ? id('BigUint') : id('Uint')), [
        param,
    ]);
}
