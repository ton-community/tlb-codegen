...
import { findNotReservedName, firstLower, getCurrentSlice, getSubStructName } from '../../utils';
...
export class TypescriptGenerator implements CodeGenerator {
    ...
    addTonCoreClassUsage(name: string) {
        this.jsCodeDeclarations.push(tImportDeclaration(id(name), tStringLiteral('@ton/core')));
    }
    ...
    addCopyCellToBuilder() {
        this.jsCodeDeclarations.push(
            tCodeAsIs(`export function copyCellToBuilder(from: Cell, to: Builder): void {
    ...
    addEmbeddedTypes() {
        this.jsCodeDeclarations.push(
            tCodeAsIs(`export interface Bool {
    readonly kind: 'Bool';
...
`),
    ...
    addTlbType(tlbType: TLBType): void {
        let typeName = findNotReservedName(firstLower(tlbType.name), '0');
        ...
        tlbType.constructors.forEach((constructor) => {
            ...
            let ctx: ConstructorContext = {
                ...
                name: findNotReservedName(firstLower(constructorTypeName), '_' + constructor.name),
                typeName: typeName,
                loadStatements: [],
                loadProperties: [tObjectProperty(id('kind'), tStringLiteral(constructorTypeName))],
                properties: [tTypedIdentifier(id('kind'), tStringLiteral(constructorTypeName))],
                storeStatements: [],
            };
            ...
            let structX = tStructDeclaration(id(constructorTypeName), ctx.properties, structTypeParametersExpr);

            constructor.constraints.forEach((constraint) => {
                this.genCodeForConstraint(constraint, typeName, tlbType, ctx);
            });

            ctx.loadStatements.push(tReturnStatement(tObjectExpression(ctx.loadProperties)));
            loadStatements = this.constructorStmtsToTypeStmts(constructor, tlbType, ctx, loadStatements);

            if (constructor.tag.bitLen != 0) {
                ctx.storeStatements.splice(0, 0, storeTagExpression(constructor.tag));
            }

            let storeStatement: Statement = storeFunctionStmt(ctx.storeStatements);

            if (tlbType.constructors.length > 1) {
                storeStatement = checkKindStmt(typeName, constructorTypeName, storeStatement);
            }
        ...
        const currentType = tTypeWithParameters(id(tlbType.name), structTypeParametersExpr);
        ...
        if (anyConstructor) {
            anyConstructor.parameters.forEach((element) => {
                if (element.variable.type == 'Type') {
                    ...
                    storeFunctionParameters.push(storeFunctionParam(element.variable.name));
                }
    ...
    private constructorStmtsToTypeStmts(
        ...
        if (constructor.tag.bitLen != 0 || tlbType.constructors.length > 1) {
            ...
            if (constructor.tag.bitLen != 0) {
                ...
                let loadBitsStatement: Statement[] = [skipTagStmt(constructor.tag.bitLen)];
            ...
            constructor.parameters.forEach((param) => {
                if (param.variable.isConst && !param.variable.negated) {
                    ...
                    conditions.push(tEqualExpression(id(argName), getParamVarExpr(param, constructor)));
                }
            });
            loadStatements.push(tIfStatement(getCondition(conditions), ctx.loadStatements));
        } else {
    ...
    private genCodeForConstraint(
        ...
        let storeConstraintAST = convertToAST(constraint, ctx.constructor, id(variableCombinatorName));
        let exceptionCommentLastPart = ` is not satisfied while loading "${getSubStructName(
            tlbType,
            ctx.constructor,
        )}" for type "${tlbType.name}"`;
        ctx.loadStatements.push(checkConstraintStmt(loadConstraintAST, exceptionCommentLastPart));
        ctx.storeStatements.push(checkConstraintStmt(storeConstraintAST, exceptionCommentLastPart));
    }
    ...
    private addVarToConstructorLoadProperty(
        ...
        if (variable.type == '#' && !variable.isField) {
            ...
            ctx.properties.push(tTypedIdentifier(id(variable.name), id('number'), isOptional));
        ...
        if (varExpr) {
            ctx.loadProperties.push(tObjectProperty(id(variable.name), varExpr));
        }
    }
    ...
    handleField(field: TLBField, slicePrefix: Array<number>, ctx: ConstructorContext) {
        ...
        if (field.subFields.length > 0) {
            inSeparateRef(slicePrefix, () => {
                ctx.loadStatements.push(sliceLoad(slicePrefix, currentSlice));
                ...
                ctx.storeStatements.push(storeRefStmt(slicePrefix, currentCell));
            });
        }

        if (field.fieldType.kind == 'TLBExoticType') {
            inSeparateRef(slicePrefix, () => {
                ctx.loadStatements.push(loadRefStmt(slicePrefix, currentSlice));
                addLoadProperty(field.name, id(getCurrentSlice(slicePrefix, 'cell')), undefined, ctx);
                ctx.properties.push(tTypedIdentifier(id(field.name), id('Cell')));
            ...
            if (fieldInfo.typeParamExpr) {
                ctx.properties.push(tTypedIdentifier(id(field.name), fieldInfo.typeParamExpr));
            }
    ...
    handleType(
        ...
        storeParametersOutside = [tMemberExpression(id(ctx.typeName), id(fieldName))]; // TODO: use only field
        ...
        if (fieldType.kind == 'TLBNumberType') {
            exprForParam = {
                ...
                argStoreExpr: convertToAST(fieldType.storeBits, ctx.constructor, id(ctx.typeName)),
                paramType: 'number',
            ...
            if (fieldType.bits instanceof TLBNumberExpr && fieldType.bits.n == 1) {
                ...
                let argStoreExpr = convertToAST(fieldType.bits, ctx.constructor, id(ctx.name));
                if (isBuffer) {
            ...
            storeParametersOutside[0] = tFunctionCall(tMemberExpression(storeParametersOutside[0], id('beginParse')), [
                id('true'),
            ]);
            storeParametersInside[0] = tFunctionCall(tMemberExpression(storeParametersInside[0], id('beginParse')), [
                id('true'),
            ]);
            ...
            exprForParam = {
                ...
                argStoreExpr: convertToAST(fieldType.n, ctx.constructor, id(ctx.name)),
                paramType: 'bigint',
            ...
            result.loadExpr = tFunctionCall(id('parseTuple'), [
            ...
            result.storeStmtInside = tExpressionStatement(
                tFunctionCall(id('copyCellToBuilder'), [
                    tFunctionCall(id('serializeTuple'), storeParametersInside),
                    id('builder'),
                ]),
            );
            result.storeStmtOutside = tExpressionStatement(
                tFunctionCall(id('copyCellToBuilder'), [
                    tFunctionCall(id('serializeTuple'), storeParametersOutside),
                    id('builder'),
                ]),
            );
            ...
            let getParameterFunctionId = id(ctx.name + '_get_' + fieldType.variableName);
            ...
            let subExprInfo: FieldInfoType = this.handleType(field, fieldType.value, true, ctx, [1, 0], argIndex);
            if (subExprInfo.loadExpr) {
            ...
            let loadFunction =
                'load' + (fieldType.value === undefined ? 'Bool' : fieldType.value ? 'BoolTrue' : 'BoolFalse');
            result.loadExpr = tFunctionCall(id(loadFunction), [id('slice')]);
            result.typeParamExpr = id('Bool');
            result.storeStmtInside = tExpressionStatement(
                tFunctionCall(tFunctionCall(id('storeBool'), storeParametersInside), [id('builder')]),
            );
            result.storeStmtOutside = tExpressionStatement(
                tFunctionCall(tFunctionCall(id('storeBool'), storeParametersOutside), [id('builder')]),
            );
            ...
            if (subExprInfo.typeParamExpr && subExprInfo.loadFunctionExpr && subExprInfo.storeFunctionExpr) {
                ...
                if (fieldType.extra && subExprInfo.loadExpr) {
                    ...
                    valueStore = dictValueStore(
                        subExprInfo.typeParamExpr,
                        subExprInfo.storeFunctionExpr,
                        extraInfo.storeFunctionExpr,
                    );

                    if (extraInfo.loadExpr) {
                        result.loadExpr = dictLoadExpr(
                            keyForLoad,
                            dictAugParse(extraInfo.loadExpr, subExprInfo.loadExpr),
                            currentSlice,
                            fieldType.directStore,
                        );
                    }
                } else {
                ...
                result.storeStmtInside = dictStoreStmt(
                    currentCell,
                ...
                result.storeStmtOutside = dictStoreStmt(
                    currentCell,
        ...
        if (result.storeStmtOutside && !result.storeFunctionExpr) {
            ...
            if (result.typeParamExpr) {
                if (
                    ...
                    result.storeFunctionExpr = storeFunctionExpr(result.typeParamExpr, result.storeStmtInside);
                } else {
    ...
    private extractFieldReferences(node: TheNode, typeName: string, fieldsUsed: Set<string>): void {
        ...
        switch (node.type) {
            ...
            case 'MemberExpression':
                if (
                    node.thisObject.type === 'Identifier' &&
