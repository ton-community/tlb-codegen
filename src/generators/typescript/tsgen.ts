...
export function tStructExpression(fields: Array<TypedIdentifier>): StructExpression {
...
function toCodeArray(nodeArray: Array<TheNode>, code: CodeBuilder, delimeter: string) {
...
export function toCode(node: TheNode, code: CodeBuilder = new CodeBuilder()): CodeBuilder {
    ...
    if (node.type == 'FunctionCall') {
        code.add(
            `${toCode(node.functionId).render()}${node.typeParameters ? toCode(node.typeParameters).render() : ''}(`,
            false,
        );
        toCodeArray(node.parameters, code, ', ');
        code.add(`)`, false);
    ...
    if (node.type == 'TernaryExpression') {
        code.add(
            `(${toCode(node.condition).render()} ? ${toCode(node.body).render()} : ${toCode(node.elseBody).render()})`,
            false,
        );
    }
