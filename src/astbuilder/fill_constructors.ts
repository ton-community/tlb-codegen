...
function fillConstraintsAndNegationVars(constructor: TLBConstructorBuild, declaration: Declaration) {
    declaration.fields.forEach((field) => {
...
function fillArgNames(tlbType: TLBTypeBuild) {
    ...
    tlbType.constructors[0]?.parameters.forEach((_element) => {
...
function compareConstructors(a: TLBConstructorBuild, b: TLBConstructorBuild): number {
...
function reorganizeWithArg(mathExpr: TLBMathExpr, argName: string, varName: string): TLBMathExpr {
    let reorganized = reorganizeExpression(
        new TLBBinaryOp(new TLBVarExpr(argName), mathExpr, '=', new Set<string>(), false),
        varName,
    );
...
function getConstructorTag(declaration: Declaration, _input: string[]): TLBConstructorTag {
    ...
    if (tag[0] == '#') {
        ...
        if (hasTrailingUnderscore) {
            const binary = parseInt(hexPart, 16)
                .toString(2)
                .padStart(hexPart.length * 4, '0');
            const truncatedBinary = binary.slice(0, -1);
            const truncatedHex = parseInt(truncatedBinary, 2)
                .toString(16)
                .padStart(Math.ceil(truncatedBinary.length / 4), '0');
            return {
...
function checkAndRemovePrimitives(
    ...
    _input: string[],
    typeDeclarations: Map<string, { declaration: Declaration; constructor: TLBConstructorBuild }[]>,
) {
    let toDelete: string[] = [];
...
function findAvailableFieldsNames(fields: TLBField[], variablesSet: Set<string>) {
...
function convertParameterToReadonly(tlbParameter: TLBParameterBuild): TLBParameter {
...
function getStringDeclaration(declaration: Declaration, input: string[]): string {
    let result = '';
...
const TypeGuards = {
    isFieldBuiltinDef: (field: FieldDefinition): field is FieldBuiltinDef => field instanceof FieldBuiltinDef,

    isFieldCurlyExprDef: (field: FieldDefinition): field is FieldCurlyExprDef => field instanceof FieldCurlyExprDef,

    isFieldAnonymousDef: (field: FieldDefinition): field is FieldAnonymousDef => field instanceof FieldAnonymousDef,

    isFieldNamedDef: (field: FieldDefinition): field is FieldNamedDef => field instanceof FieldNamedDef,

    isFieldExprDef: (field: FieldDefinition): field is FieldExprDef => field instanceof FieldExprDef,

    isCompareExpr: (expr: Expression): expr is CompareExpr => expr instanceof CompareExpr,

    isMathExpr: (expr: Expression): expr is MathExpr => expr instanceof MathExpr,

    isNegateExpr: (expr: Expression): expr is NegateExpr => expr instanceof NegateExpr,

    isNameExpr: (expr: Expression): expr is NameExpr => expr instanceof NameExpr,

    isNumberExpr: (expr: Expression): expr is NumberExpr => expr instanceof NumberExpr,

    isCellRefExpr: (expr: Expression): expr is CellRefExpr => expr instanceof CellRefExpr,

    isBuiltinZeroArgs: (expr: Expression): expr is BuiltinZeroArgs => expr instanceof BuiltinZeroArgs,

    isBuiltinOneArgExpr: (expr: Expression): expr is BuiltinOneArgExpr => expr instanceof BuiltinOneArgExpr,

    isCombinatorExpr: (expr: Expression): expr is CombinatorExpr => expr instanceof CombinatorExpr,

    isCondExpr: (expr: Expression): expr is CondExpr => expr instanceof CondExpr,
};
...
function formatAnonymousField(field: FieldAnonymousDef): string {
    const innerFields = field.fields
        .map((innerField, index) => (index > 0 ? ' ' + formatField(innerField) : formatField(innerField)))
        .join('');
