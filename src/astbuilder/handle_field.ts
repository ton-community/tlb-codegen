...
function getField(field: FieldDefinition, constructor: TLBConstructorBuild, fieldIndex: string): TLBField | undefined {
    ...
    if (field instanceof FieldNamedDef || field instanceof FieldExprDef) {
        ...
        if (field.expr instanceof CellRefExpr) {
            if (
                ...
                (field.expr.expr.name == 'MERKLE_UPDATE' || field.expr.expr.name == 'MERKLE_ROOT')
            ) {
                return {
                    name: fieldName,
                    anonymous: true,
