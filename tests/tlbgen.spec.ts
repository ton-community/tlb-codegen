...
import { BitString, Slice, Builder, Cell } from 'ton'
...
import { loadBlock } from '../generated_block'
...
function logCell(cs: Slice) {
    let res = ''
    while(cs.remainingBits >= 4) {
        res += cs.loadUint(4).toString(16);
    }
    if (cs.remainingBits) {
        res += cs.loadUint(cs.remainingBits).toString(16);
    }
    while(cs.remainingRefs) {
        if (!cs.loadRef().isExotic) {
            res += '\n\t' + logCell(cs.loadRef().beginParse())
        } else {
            cs.loadRef();
            res += '\n\texotic' 
        }
...
describe('Generating tlb code', () => {
    test('Basic types', () => {
        ...
        const cs = Cell.fromBase64(state).beginParse()
        // console.log(logCell(cs))
        // fs.writeFile('kek' + '.txt', Cell.fromBase64(state).toString(), () => { });

        let blk = loadBlock(cs);
        console.log(blk)
        ...
        let falseAnonField: FalseAnonField = {kind: 'FalseAnonField', value: 3}
